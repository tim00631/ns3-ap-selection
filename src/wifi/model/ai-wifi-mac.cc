...
NS_LOG_COMPONENT_DEFINE ("AiWifiMac");

NS_OBJECT_ENSURE_REGISTERED (AiWifiMac);

TypeId
AiWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::AiWifiMac")
    .SetParent<InfrastructureWifiMac> ()
    .SetGroupName ("Wifi")
    .AddConstructor<AiWifiMac> ()
    .AddAttribute ("ProbeRequestTimeout", "The duration to actively probe the channel.",
                   TimeValue (Seconds (0.05)),
                   MakeTimeAccessor (&AiWifiMac::m_probeRequestTimeout),
                   MakeTimeChecker ())
    .AddAttribute ("WaitBeaconTimeout", "The duration to dwell on a channel while passively scanning for beacon",
                   ...
                   MakeTimeAccessor (&AiWifiMac::m_waitBeaconTimeout),
                   MakeTimeChecker ())
    .AddAttribute ("AssocRequestTimeout", "The interval between two consecutive association request attempts.",
                   TimeValue (Seconds (0.5)),
                   MakeTimeAccessor (&AiWifiMac::m_assocRequestTimeout),
                   MakeTimeChecker ())
    .AddAttribute ("MaxMissedBeacons",
                   "Number of beacons which much be consecutively missed before "
                   "we attempt to restart association.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&AiWifiMac::m_maxMissedBeacons),
    ...
    .AddAttribute ("ActiveProbing",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&AiWifiMac::SetActiveProbing, &AiWifiMac::GetActiveProbing),
                   MakeBooleanChecker ())
    .AddTraceSource ("Assoc", "Associated with an access point.",
                     MakeTraceSourceAccessor (&AiWifiMac::m_assocLogger),
                     "ns3::Mac48Address::TracedCallback")
    .AddTraceSource ("DeAssoc", "Association with an access point lost.",
                     MakeTraceSourceAccessor (&AiWifiMac::m_deAssocLogger),
                     "ns3::Mac48Address::TracedCallback")
    .AddTraceSource ("BeaconArrival",
                     "Time of beacons arrival from associated AP",
                     MakeTraceSourceAccessor (&AiWifiMac::m_beaconArrival),
...
void
AiWifiMac::SetActiveProbing (bool enable)
{
...
bool
AiWifiMac::GetActiveProbing (void) const
{
  return m_activeProbing;
}

void
AiWifiMac::SetWifiPhy (const Ptr<WifiPhy> phy)
{
  NS_LOG_FUNCTION (this << phy);
  RegularWifiMac::SetWifiPhy (phy);
  m_phy->SetCapabilitiesChangedCallback (MakeCallback (&AiWifiMac::PhyCapabilitiesChanged, this));
}

void
AiWifiMac::SetWifiRemoteStationManager (const Ptr<WifiRemoteStationManager> stationManager)
{
...
void
AiWifiMac::SendAssociationRequest (bool isReassoc)
{
  ...
  m_assocRequestEvent = Simulator::Schedule (m_assocRequestTimeout,
                                             &AiWifiMac::AssocRequestTimeout, this);
}

void
AiWifiMac::SendCfPollResponse (void)
{
  NS_LOG_FUNCTION (this);
...
void
AiWifiMac::TryToEnsureAssociated (void)
{
  NS_LOG_FUNCTION (this);
  switch (m_state)
    {
...
void
AiWifiMac::ScanningTimeout (void)
{
  NS_LOG_FUNCTION (this);
  if (!m_candidateAps.empty ())
    {
      NewApInfo bestAp = m_candidateAps.front();
...
void
AiWifiMac::AssocRequestTimeout (void)
{
  NS_LOG_FUNCTION (this);
  SetState (WAIT_ASSOC_RESP);
  SendAssociationRequest (false);
...
void
AiWifiMac::RestartBeaconWatchdog (Time delay)
{
  ...
  if (Simulator::GetDelayLeft (m_beaconWatchdog) < delay
      && m_beaconWatchdog.IsExpired ())
    {
      ...
      m_beaconWatchdog = Simulator::Schedule (delay, &AiWifiMac::MissedBeacons, this);
    }
}

bool
AiWifiMac::IsAssociated (void) const
{
  return m_state == ASSOCIATED;
}

bool
AiWifiMac::IsWaitAssocResp (void) const
{
  return m_state == WAIT_ASSOC_RESP;
}

void
AiWifiMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
...
void
AiWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
...
void
AiWifiMac::UpdateCandidateApList (NewApInfo newApInfo)
{
  ...
  for (std::vector<NewApInfo>::iterator i = m_candidateAps.begin(); i != m_candidateAps.end(); ++i)
    {
...
void
AiWifiMac::UpdateApInfoFromBeacon (MgtBeaconHeader beacon, Mac48Address apAddr, Mac48Address bssid)
{
...
void
AiWifiMac::UpdateApInfoFromProbeResp (MgtProbeResponseHeader probeResp, Mac48Address apAddr, Mac48Address bssid)
{
...
void
AiWifiMac::UpdateApInfoFromAssocResp (MgtAssocResponseHeader assocResp, Mac48Address apAddr)
{
...
SupportedRates
AiWifiMac::GetSupportedRates (void) const
{
...
AiWifiMac::GetCapabilities (void) const
{
...
void
AiWifiMac::SetState (MacState value)
{
  if (value == ASSOCIATED
      && m_state != ASSOCIATED)
    {
...
void
AiWifiMac::SetEdcaParameters (AcIndex ac, uint32_t cwMin, uint32_t cwMax, uint8_t aifsn, Time txopLimit)
{
...
void
AiWifiMac::PhyCapabilitiesChanged (void)
{
  NS_LOG_FUNCTION (this);
  if (IsAssociated ())
    {

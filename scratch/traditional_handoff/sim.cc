...
void GatherApInfo (Ptr<Node> targetStaNode) {
	...
	if (wifi_mac->IsAssociated()) {
		...
		ns3AI.step(ConvertMacAddressToIndex(wifi_mac->GetBssid()), throughput);
	}
	else {
		std::cout << "Not associated...\n";
		ns3AI.step(-1, throughput);
	}
			 			  ...
			 			  double referenceDistance)
{
	...
	// print config
	// Config::SetDefault("ns3::ConfigStore::Filename", StringValue(cwd+"/output-attributes.txt"));

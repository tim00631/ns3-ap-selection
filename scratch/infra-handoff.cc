...
int main (int argc, char *argv[])
{
  ...
  // DEVICES
  NetDeviceContainer switchLanDevices; // APs + serv
  ...
  // Config::SetDefault("ns3::DropTailQueue::MaxPackets", UintegerValue(10));
	...
	for (uint32_t i = 0; i < apNodes.GetN(); i++) {
		...
		switchLanDevices.Add(link.Get(0));
  ...
  serverInterface = ip.Assign(switchLanDevices.Get(nWifis)); 
  ...
  for (uint32_t ap = 0; ap < nWifis; ++ap){
      ...
      bridgeDev = bridge.Install (apNodes.Get (ap), 
                                  NetDeviceContainer (apDev, switchLanDevices.Get (ap)));
      ...
      */
      ...
      wifiPhy.EnablePcap ("sta", staDev);
      SetPosition (apNodes.Get(ap),  Vector (ap*150.0, 0.0, 0.0));
      wifiPhy.EnablePcap ("wifi-ap", apDev);
